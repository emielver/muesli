...
- **AI meeting notes** — BYOK with OpenAI or OpenRouter, sign in with your ChatGPT Plus/Pro subscription (no API key needed), or use local Ollama models. Custom LLM endpoints can optionally resolve short-lived credentials with an **API Key Command** and attach additional headers for gateway routing, authentication, or metadata. Commands run via `/bin/sh` with your user permissions; use absolute executable paths. Non-empty command output takes precedence over the static API key, while failures fall back to it. Auto-generated meeting titles. Re-summarize any meeting.
